...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_var2 (y: string) (Neq: x <> y) :
      substi s x (tm_var y) (tm_var y)
  | s_abs1 (t1: tm) (T: ty) :
      substi s x <{\x:T, t1}> <{\x:T, t1}>
  | s_abs2 (y: string) (t1 t1': tm) (T: ty) (Neq: x <> y) (H: substi s x t1 t1') :
      substi s x <{\y:T, t1}> <{\y:T, t1'}>
  | s_app (t1 t1' t2 t2': tm) (H1: substi s x t1 t1') (H2: substi s x t2 t2') :
...
Proof.
  intros s x t t'. split; intro H.
  - generalize dependent t'.
    induction t; intros; simpl in H; subst.
    + destruct ((x =? s0)%string) eqn: Eqx.
      * rewrite eqb_eq in Eqx. rewrite Eqx. apply s_var1.
      * apply s_var2. rewrite <- eqb_neq. apply Eqx.
    + apply s_app.
      * apply IHt1. reflexivity.
      * apply IHt2. reflexivity.
    + destruct ((x =? s0)%string) eqn: Eqx.
      * rewrite eqb_eq in Eqx. rewrite Eqx. apply s_abs1.
      * apply s_abs2. 
  ...
  - induction H; simpl;
    ...
    try (rewrite <- eqb_neq in Neq; rewrite Neq);
...
Proof.
  ...
  clear H1 H3. induction T2.
  - discriminate.
  - inversion H0. rewrite H2 in H1.
